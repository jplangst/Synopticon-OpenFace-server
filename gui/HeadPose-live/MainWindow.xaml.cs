...
namespace HeadPoseLive
{
    ...
    public partial class MainWindow : Window
    {
        ...
        //private IWampSubject EyeDataSubject = null;
        //private IWampSubject HeadDataSubject = null;
        private IWampSubject OpenFaceSubject = null;
        private int counter = 0;
        private int NatNetID = new Random().Next();

        //private ConcurrentQueue<EyeDataStruct> EyeDataSamples = null;
        //private ConcurrentQueue<List<float>> HeadDataSamples = null;
        private ConcurrentQueue<OpenFaceDataStruct> OpenFaceDataSamples = null;
        private bool StopWAMP = false;

        Thread WAMPThread = null;
        ...
        private float[] ConvertEulerAnglesToQuaternion(List<float> EulerAngles)
        {
            float Roll = EulerAngles[3];
            float Pitch = EulerAngles[4];
            float Yaw = EulerAngles[5];
            //float Roll = (float)(EulerAngles[3] * 180 / Math.PI);
            //float Pitch = (float)(EulerAngles[4] * 180 / Math.PI);
            //float Yaw = (float)(EulerAngles[5] * 180 / Math.PI);

            //int Yaw = (int)(EulerAngles[4] * 180 / Math.PI + 0.5);
            //Yaw = Math.Abs(Yaw);

            //int Roll = (int)(EulerAngles[5] * 180 / Math.PI + 0.5);
            //Roll = Math.Abs(Roll);

            //int Pitch = (int)(EulerAngles[3] * 180 / Math.PI + 0.5);
            //Pitch = Math.Abs(Pitch);
            //Yaw = Math.Abs(Yaw);
            //Roll = Math.Abs(Roll);
            //Pitch = Math.Abs(Pitch);

            float W = (float) (Math.Cos(Roll / 2) * Math.Cos(Pitch / 2) * Math.Cos(Yaw / 2) + 
                Math.Sin(Roll/2)*Math.Sin(Pitch/2)*Math.Sin(Yaw/2));

            float X = (float)(Math.Cos(Roll / 2) * Math.Sin(Pitch / 2) * Math.Cos(Yaw / 2) +
                Math.Sin(Roll / 2) * Math.Cos(Pitch / 2) * Math.Sin(Yaw / 2));

            float Y = (float)(Math.Cos(Roll / 2) * Math.Cos(Pitch / 2) * Math.Sin(Yaw / 2) -
                Math.Sin(Roll / 2) * Math.Sin(Pitch / 2) * Math.Cos(Yaw / 2));

            float Z = (float)(Math.Sin(Roll / 2) * Math.Cos(Pitch / 2) * Math.Cos(Yaw / 2) -
                Math.Cos(Roll / 2) * Math.Sin(Pitch / 2) * Math.Sin(Yaw / 2));

            float[] Quaternion = new float[] { X, Y, Z, W };
            return Quaternion;
        }

        private float Distance(Tuple<float, float, float> p1, Tuple<float, float, float> p2)
        {
            float d1 = p1.Item1 - p2.Item1;
            float d2 = p1.Item2 - p2.Item2;
            float d3 = p1.Item3 - p2.Item3;
            return (float)Math.Sqrt(d1 * d1 + d2 * d2 + d3 * d3);
        }

        private Tuple<float, float, float> AverageBetweenTuples(Tuple<float, float, float> p1, Tuple<float, float, float> p2)
        {
            return new Tuple<float, float, float>((p1.Item1 + p2.Item1) / 2, (p1.Item2 + p2.Item2) / 2, (p1.Item2 + p2.Item2) / 2);
        }
        ...
        public MainWindow()
        {
            //AllocConsole();
            InitializeComponent();
            //Console.WriteLine("Hello from the console's side");
            ...
            if (cam_select.camera_selected)
            {
                ...
                StartExperiment();
                WAMPThread = new Thread(new ThreadStart(CreateWAMPClient));
                WAMPThread.Start();
            }
            else
            {
        ...
        private void VideoLoop(UtilitiesOF.SequenceReader reader)
        {
            ...
            while (running)
            {
                ...
                EyeDataStruct EyeSample = new EyeDataStruct(); ;
                if (detectionSucceeding)
                {
                    ...
                    {
                        EyeSample.Confident = face_model.GetConfidence();
                        EyeSample.EyeLandmarks3D = face_model.CalculateAllEyeLandmarks3D(reader.GetFx(), reader.GetFy(), reader.GetCx(), reader.GetCy());
                        EyeSample.Gaze = gaze_analyser.GetGazeCamera();
                    }
                ...
                try
                {
                    Dispatcher.Invoke(DispatcherPriority.Render, new TimeSpan(0, 0, 0, 0, 200), (Action)(() =>
                    {
                        ...
                        if (IsBroadcastingToWAMP)
                        {
                            OpenFaceDataStruct Sample = new OpenFaceDataStruct();
                            Sample.GazeData = EyeSample;
                            Sample.HeadData = pose;
                            OpenFaceDataSamples.Enqueue(Sample);
                        }
        ...
        public struct EyeDataStruct
        {
            ...
            public List<Tuple<float, float, float>> EyeLandmarks3D;
            public Tuple<Tuple<float, float, float>, Tuple<float, float, float>> Gaze;
        }

        public struct HeadDataStruct
        {
            public string RigidBodyID;
            public Tuple<float, float, float> Position;
            public Tuple<float, float, float> Orientation;
        }

        public struct OpenFaceDataStruct
        {
            public EyeDataStruct GazeData;
            public List<float> HeadData;
        }
    }
}